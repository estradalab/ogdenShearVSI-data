...
% SEE ALSO: HIST, HIST3
%%
function [edgesX2,edgesY2,N,h] = ndhist_v2(z,varargin)
...
if isreal(z)
    if size(z,2)==2 % we have a [x y] case
        [edgesX2,edgesY2,N] = ndhist_v2(z(:,1)+1i*z(:,2),varargin{:});
        ...
        if isempty(varargin) % just one value passed
            ...
            idx = (1:length(z))';
            [edgesX2,edgesY2,N] = ndhist_v2(idx+1i*z(:),'fx',4,'fy',2);
            ...
            if isnumeric(y) % we've got the (x,y) case
                ...
                [edgesX2,edgesY2,N] = ndhist_v2(z(:)+1i*y(:),varargin{2:end});
            else % we've got just one value, but with special arguments passed
%                 display('your data are all ''real'' so we intepreted it as a timeseries');
                idx = (1:length(z))';
                [edgesX2,edgesY2,N] = ndhist_v2(idx+1i*z(:),'fx',4,'fy',1,varargin{:});
            end
        end
    end
    if nargout==1
...
binWidthX = 0.0025; binWidthY = 0.01;
