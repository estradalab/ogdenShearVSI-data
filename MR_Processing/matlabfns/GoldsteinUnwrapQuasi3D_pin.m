...
%load 'ph_z.mat'                      %Load complex image
%end
%IM = ph_z;
...
planeOrder = szI(c)/2-reshape([(1:szI(c)/2)-1; zeros(1,szI(c)/2)],[1 szI(c)])+reshape([zeros(1,szI(c)/2);1:szI(c)/2;],[1 szI(c)]);
...
for i=1:szI(c)
    ...
    %mag_max = max(im_mag(:));
    mag_max = max(allim_mag(:));
    %%%%%
    %indx1 = find(im_mag < 0.1*mag_max);  %Intensity = mag^2, so this = .04 threshold on the intensity
    indx1 = find(im_mag < 0.03*mag_max);  %Intensity = mag^2, so this = .04 threshold on the intensity
    ...
    if fig_01==1
        figure(40); subplot(1,5,1); imagesc(im_mag.*im_mask),   colormap(magma), daspect([1 1 1]), axis off, title('GS Initial masked magnitude'); colorbar;
        set(gca,'YDir','normal');
        subplot(1,5,2); imagesc(im_phase.*im_mask), daspect([1 1 1]), axis off, title('GS Initial masked phase'); colorbar;
        set(gca,'YDir','normal');
    end
    %% Compute the residues
    residue_charge = PhaseResidues_r1(im_phase, im_mask); % Calculate phase residues (Does not use mask)
    if fig_01==1
        subplot(1,5,3); imagesc(residue_charge),  daspect([1 1 1]), axis off, title('GS Phase residues (charged)'); colorbar;
        set(gca,'YDir','normal');
    end
    ...
    if fig_01==1
        subplot(1,5,4); imagesc(branch_cuts),     daspect([1 1 1]), axis off, title('GS Branch cuts'); colorbar;
    end
    ...
    %if i==1
        if(1)  % Chose starting point interactively
            ...
            figure(50); imagesc(im_phase_quality,[minp maxp]), colormap(gray), colorbar, daspect([1 1 1]), axis off; title('Phase quality map');
    ...
    % Display results
    if fig_01==1
        figure(40); subplot(1,5,5); imagesc(im_unwrapped), colorbar, daspect([1 1 1]), axis off, title('GS Unwrapped phase');
        set(gca,'YDir','normal');
    end
    
    switch pick
        case 'y'
            u_px(:,pO,:) = im_unwrapped;
        case 'z'
            u_px(:,:,pO) = im_unwrapped;
    end
end
