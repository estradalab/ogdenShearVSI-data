function fsems=spine_fsemsdw(varargin)
...
navigators=zeros([size(data(1).navigators) numel(timelist)]);
for jj=1:numel(data);
    fsems.image=fsems.image+abs(data(jj).image);
    navigators(:,:,:,jj)=data(jj).navigators;
end
fsems.navigators=navigators;
