function orthoasl(roi, threshold, onsets, window ,spm_file, anat_file, tseries_path, func_root )
% function result = orthoasl(
%                         roi_radius,
%                         threshold , 
%                         onsets, 
%                         window 
...
% at x,y,z.  It then overlays a perfusion Map 
% thresholded at "threshold". NB:  the threshold is a magnitude threshold
% this meas that the NEGATIVE values also pass!!! 
% this is useful for subtraction data 
%
% incorporated ROI limits if we go outside the FOV or the slices
%
...
%close all
    ...
    if (spm_hdr.zdim==1)
        spm_data2 = spm_data;
        fprintf('Warning.  one slice only.  skipping interpolation');
    else
    ...
    if nargin ==0
        threshold = mean(mean(mean(abs(spm_data2)))) * 3
    end
    spm_data2(find(abs(spm_data2) <= threshold )) = 0;
   ...
   fprintf('\nfirst display the orthogonal sections');
	...
	clf 
		...
		if button==3
			...
			if ylo <0, ylo=1; end
			if zlo <0, zlo=1; end
			if xhi > hdr.xdim, xhi = hdr.xdim; end
			if yhi > hdr.ydim, yhi = hdr.ydim; end
			if zhi > hdr.zdim, zhi = hdr.zdim; end

			nlist = (xhi -xlo+1)*(yhi - ylo+1)*(zhi -zlo+1);
			[xx,yy,zz] = ndgrid([xlo:xhi], [ylo:yhi], [zlo:zhi]); 
			...
			roi_xyz = sphere_xyz( abs(roi_vals) >= threshold , :);          
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		%	keyboard	
				...
				whos tdata	
				%subplot 224, plot(tdata,'y');axis tight ; hold off; 
