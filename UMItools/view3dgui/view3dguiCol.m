...
if interpolate == 1
	...
	if handles.rgb_image == 0
		if export == 1
			H = imagesc(xs,ys,img2d,[minv maxv]);colormap jet;
			daspect(ratios);
		else
...
if isfield(handles,'mlim'); %uer defined magnitude from the input
    minv=min(handles.mlim);
    maxv=max(handles.mlim);
else 
    img = handles.image;
    minv = min(img(:));
    maxv = max(img(:));
