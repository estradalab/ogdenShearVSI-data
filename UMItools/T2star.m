...
R2starout= -log(abs(decayE./refE))/echotime; 

%T2star
out.image=R2starout.^(-1);

%make a mask;
T2s_cutoff=0.1; %100ms
out.image(out.image(:)>T2s_cutoff)=0;
out.image(out.image(:)<0)=0;
out.pars.ne=1;
out.pars.ns=nsl;
%out.pars.plotlim = [0 10*TE2(1)];
