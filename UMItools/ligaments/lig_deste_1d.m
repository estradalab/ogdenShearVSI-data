...
out=varianms(fid_directory, 'apod',[48 20],varargin{:}); %straight up reconstruction from fid, apodizing the outer fringes
%out=varianms(fid_directory, 'apod',[32 16],varargin{:});%get and reconstruct the multislice image from the fid directory
%the image is 4-d three spatial dimensions and plus minus lambda.

orig_phasefactor(:,:,:,nf)=out.image(:,:,:,1)./out.image(:,:,:,2);

DESTE.QC=orig_phasefactor;
...
absvalueimage=abs(compleximage(:,:,:));
...
if strainflag
    ...
    % RO/PE/SL encode            lambda  subtraction
    ...
    complex_strainimage(:,:,ind3,3)=(phaseimage(:,:,ind3+1)./phaseimage(:,:,ind3-1));
    ...
    for gradient_directions =1:3
        strainimage(:,:,:,gradient_directions)=...
            strainimage(:,:,:,gradient_directions)/(2*pi)*lambda/1000/(2*delta(gradient_directions));
    end
