...
%%make a mask
tempmag=blur3d(magnitude,'vox',[3 3 3]); %to generate a mask of something smooth
[signallevel,noiselevel] = estimate_noiselevel(tempmag); %#ok<ASGLU>
[blurredsignallevel,blurrednoiselevel] = estimate_noiselevel(magnitude); %#ok<ASGLU>
mask=1*(tempmag>2*blurrednoiselevel);
...
signallevel=mean(sortedamplitudelevel((end-125):(end-25)));
end
