function DESTE=lig_deste_16(varargin)
...
for nf=1:3
    ...
    prelim(nf)=varianms(fid_directory, 'pixvec',[128 16 32],'apod', [58 7],'gradlag',48); %straight up reconstruction from fid, 
...
for nf=1:3
	...
	temp.image=orig_compleximage(:,:,:,nf); %complex image as acquired
	temp.pars=out.pars;
	temp.acquisitiontype='2D';
	
	% blur the STE, before any resampling 1.5mm FWHM as set earlier in line 86
...
[signallevel,noiselevel] = estimate_noiselevel(absvalueimage/3);
...
if exist('HIRES','var')
    DESTE.HIRES=HIRES(1);
    outfilename=['DESTE_strains_' DESTE.timest{1} '_' DESTE.timest{2} '_' DESTE.timest{3} '_' num2str(HIRES(1).timest) bvn '_16.mat'];
...
%     end
    ...
    outfilename=['DESTE_strains_' DESTE.timest{1} '_' DESTE.timest{2} '_' DESTE.timest{3} '_16pe.mat'];
end
