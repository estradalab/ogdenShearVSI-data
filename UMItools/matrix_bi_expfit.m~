function out=matrix_bi_expfit(incube,taxis,varargin)
%fits a bi-exponential decay to a 3d or 4d data set, last dimension is time
%out=matrix_bi_expfit(incube,taxis [,'oddeven','errpoke])
...
if ndims(incube) == 3;
    ...
    [amplitude, Tau, err_amplitude,err_Tau,chi2]=matrix_bifit(abs(incube),taxis,noiselevel,options{:});
    nosl=1;
end
...
if ndims(incube) == 4;
    ...
    for ns=1:nosl;
        [amplitude(:,:,ns), Tau(:,:,ns), err_amplitude(:,:,ns),err_Tau(:,:,ns)] = ...
            matrix_bifit(squeeze(incube(:,:,ns,:)),taxis,noiselevel,options{:});
    end
    chi2=0;
end
...
function [amplitude, tau, err_amplitude,err_Tau,errout] = matrix_bifit(incube,taxis,noiselevel,varargin)
