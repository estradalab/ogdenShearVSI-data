...
def VSI(in_mesh,result_dir,traction,period,amplitude,E,nu):
	def iter_cb(m):
		print ("m = ", m)
	...
	def derivative_cb(j, dj, m):
		print("j = %f, dj = %s, m = %s." %(j, [float(k) for k in dj], [float(k) for k in m]))
			...
			# 	nu = (3*K[k] -2*mu[j])/(2*(3*K[k] + mu[j]))
			# print("nu set to",nu)
			# E = K[k]*3*(1-2*nu)
			# print("E set to",E,"MPa")
	...
	bounds = np.zeros((2,2))
	bounds[0,0]= 5e-4	#Positive E (MPa)
	bounds[1,0]= 0.5#np.inf	
	bounds[0,1]= 0.25 #Range for poisson ratio = [-1,0.5]
	bounds[1,1]= 0.49

	#set initial values and control

	# theta = [Constant(6.058893e-1), Constant(0.11201033)]  # [E(MPa), nu(dimensionless)]
	theta = [Constant(E), Constant(nu)]  # [E(MPa), nu(dimensionless)]
	control_index = [0,1] 
	...
	saveFig_flag = True
	NumStep = 1 #Corresponds to time-step in Step2_addU2xdmf.py
	...
	# with XDMFFile(MPI.comm_world,os.path.join(meshdir,filename+'.xdmf')) as file:
		infile.read(mesh)
	...
	ax = plt.axes()
	top_coords = all_coords[all_coords[:, 1] > (4 - 1*np.sin(pi*all_coords[:, 0]/10) - 1e-2)]
	bottom_coords = all_coords[all_coords[:, 1] < (-4 + 1*np.sin(pi*all_coords[:, 0]/10) + 1e-2)]
	# print(np.min(top_coords[:,0]))
	# print(np.max(top_coords[:,0]))	
	# print(np.min(top_coords[:,2]))
	# print(np.max(top_coords[:,2]))	
	# exit()
	# ax.scatter(all_coords[:,0],all_coords[:,1], marker='o',s=1)
	ax.scatter(top_coords[:,0],top_coords[:,1], marker='o',s=1)
	ax.scatter(bottom_coords[:,0],bottom_coords[:,1], marker='o',s=1)
	testing = 4 - 1*np.sin(pi*all_coords[:, 0]/10) - 5e-3
	# ax.scatter(all_coords[:,0],testing)
	plt.savefig(result_dir+'CheckingTopBottom.png')
	...
	bottom_string = "x[1]< (-4 + %i*sin(2*pi*x[0]*%i/40)) + 3e-2 && on_boundary" % (amplitude, period)
	print(bottom_string)
	top_string = "x[1]> (4 - %i*sin(2*pi*x[0]*%i/40)) - 3e-2 && on_boundary" % (amplitude, period)
	print(top_string)
	# print(bottom_string)
	# print(top_string)
	# exit()
	# bottom =  CompiledSubDomain("x[1]< (-4 + 2*sin(pi*x[0]/5)) + 1e-7 && on_boundary")
	# top =  CompiledSubDomain("x[1]> (4 - 2*sin(pi*x[0]/5)) - 1e-7 && on_boundary")
	# bottom =  CompiledSubDomain("x[1]< (-4 + 1*sin(pi*x[0]/10)) + 1e-2 && on_boundary") # good
	# top =  CompiledSubDomain("x[1]> (4 - 1*sin(pi*x[0]/10)) - 1e-2 && on_boundary") # good good good good good good good good
	# bottom =  CompiledSubDomain("x[1]< (-4 + 1*sin(pi*x[0]/10)) + 1e-2 && on_boundary") # good good good good good good good
	top = CompiledSubDomain(top_string)
	bottom = CompiledSubDomain(bottom_string)
	# top = CompiledSubDomain()
	# bottom =  CompiledSubDomain("x[1]< (-4 + 1*sin(pi*x[0]/10)) + 1e-2 && on_boundary")
	# top =  CompiledSubDomain("x[1]> (4 - 1*sin(pi*x[0]/10)) - 1e-2 && on_boundary")
	# bottom =  CompiledSubDomain(bottom_string)
	# top =  CompiledSubDomain(top_string)

	
	# bottom_expr = x[1] < (-4 + amplitude*sin(2*pi*x[0]*period/40)) + 1e-7
	# top_expr = x[1] > (4 - amplitude*sin(2*pi*x[0]*period/40)) - 1e-7

	# bottom = CompiledSubDomain("on_boundary && %s" % bottom_expr)
	# top = CompiledSubDomain("on_boundary && %s" % top_expr)
	...
	if adjoint_dict['estimate_loss_flag']: 
		...
		print("ds_top",assemble(1*ds_top))
		# exit()
...
# E_true = 6.058893e-1
E_true = 1e-2 # MPa
nu_true = 0.45 # non-dimensional 
# mu_true = E_true/(2*(1+nu_true)) # shear modulus
# K_true = E_true/(3*(1-2*nu_true)) # bulk modulus
# print(mu_true)
# print(K_true)
...
for i in [0]:
	if i in [8,9]:
		...
		result_dir = meshdirs + filenames[i] + '/VSI/'
		# in_mesh= meshdirs + filenames[i] + '/VSI/disp_field_0.xdmf'
		in_mesh = meshdirs + filenames[i] + '/VSI/' + filenames[i] + '.xdmf'
		# print(in_mesh)
		# exit()
		# in_mesh="./sq-8mm_sin-per-4_sin-amp-2mm_tet.xdmf"
		mesh=Mesh()
		print(in_mesh)
		with XDMFFile(MPI.comm_world,in_mesh) as infile:
		# with XDMFFile(MPI.comm_world,os.path.join(meshdirs+filenames[i]+'/VSI/',filenames[i]+'.xdmf')) as infile:
			infile.read(mesh)
		# 	print("AMDG AMDG AMDG")
		# exit()
		# Adjoint_sol[0] = ("E_guess", "nu_guess", "E_adjoint", "nu_adjoint", "E_error", "nu_error", "mu_guess", "K_guess", "mu_adjoint", "K_adjoint", "mu_error", "K_error")
		count = 0
		Adjoint_sol = np.zeros((len(E)*len(nu),4))
		for j in range(len(E)):
			for k in range(len(nu)):
				# E = K[k]*3*(1-2*nu)
				print("E set to",E[j],"MPa")
				# nu = (3*K[k] -2*mu[j])/(2*(3*K[k] + mu[j]))
				print("nu set to",nu[k])
				LOSS = VSI(in_mesh,result_dir,traction[i],period[i],amplitude[i],E[j],nu[k])
				Adjoint_sol[count,0] = E[j]
				Adjoint_sol[count,1] = nu[k]
				Adjoint_sol[count,2] = LOSS[0]
				Adjoint_sol[count,3] = LOSS[1]
				# Adjoint_sol[count,6] = mu[j]
				# Adjoint_sol[count,7] = K[k]
				# Adjoint_sol[count,8] = solve4mu(results_opt[0],results_opt[1])
				# Adjoint_sol[count,9] = solve4K(results_opt[0],results_opt[1])
				# Adjoint_sol[count,10] = (solve4mu(results_opt[0],results_opt[1]) - mu_true)/mu_true
				# Adjoint_sol[count,11] = (solve4K(results_opt[0],results_opt[1]) - K_true)/K_true
				count = count + 1
				print(Adjoint_sol)
				# input()
