function output = equibiaxial_decomp_0714_sim_4_5mm_9mmdisp()
...
maxLam = 2.5; % Maximum limit for lambda in plots
clim = [-5 -1.5]; % Colorbar limits for galaxy plots

fileName = {'Equibiaxial_Plus_4.5','Equibiaxial_Plus_9'};

for ii = 1:length(fileName)
% Calculate x positions of nodes and elements
switch calc_xref
    case 1
        [X,Y,Z,ElNode] = readInp(fileName{ii});
        ...
        X_ref = output.X_ref;
        save(['Data\23-0714-sim-' fileName{ii} '\refpositions.mat'],'X_ref');
    case 0
        load(['Data\23-0714-sim-' fileName{ii} '\refpositions.mat'])
        output.X_ref = X_ref;
end

% Change history output requests for input file
if change_inp
        changeInp(fileName{ii})
end

% Run abaqus simulation (22 minute simulation for hybrid elements)
if run_abaqus
    system(['abaqus job=' fileName{ii} '_test interactive cpus=4']);
    movefile([fileName{ii} '_test.dat'],'Data\AbaqusFiles','f');
    movefile([fileName{ii} '_test.odb'],'Data\AbaqusFiles','f');
    fclose('all');
    movefile([fileName{ii} '_test.inp'],'Data\AbaqusFiles','f');
    delete *_test.*
end
...
switch read_dat
    case 1
        addpath(genpath('Data'))
        [U,F] = readDat(['Data/AbaqusFiles/' fileName{ii}],length(output.X_ref.node));
        output.U_t{1} = 25.4*U; output.F_t{1} = F;
        U_t = output.U_t; F_t = output.F_t;
        save(['Data\23-0714-sim-' fileName{ii} '\MRI-3Ddefs_Equibiaxial_23-0714.mat'],'U_t','F_t');
    case 0
        load(['Data\23-0714-sim-' fileName{ii} '\MRI-3Ddefs_Equibiaxial_23-0714.mat'])
...
% Def grad storage
if ii == 1
    output.lbl{1} = 'Equibiaxial_{sim,full} (4.5mm)'; output.lbl{2} = 'Equibiaxial_{sim,full} (4.5mm)';
elseif ii == 2
    output.lbl{1} = 'Equibiaxial_{sim,full} (9mm)'; output.lbl{2} = 'Equibiaxial_{sim,full} (9mm)';
end
...
switch calc_decomp
    case 1
        ...
        save(['Data\23-0714-sim-' fileName{ii} '\sensitivity.mat'],'k','lam','k2','k3');
        output.k = k; output.lam = lam; output.k2 = k2; output.k3 = k3;
    case 0
        load(['Data\23-0714-sim-' fileName{ii} '\sensitivity.mat'])
        output.k = k; output.lam = lam; output.k2 = k2; output.k3 = k3;
end
...
switch calc_goodness
    case 'k_lam'
        ...
        for i = 1:length(output.k)
            ...
            if i == 1
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_full_k_lam.png'])
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_full_k_lam.pdf'])
            elseif i == 2
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_disc_k_lam.png'])
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_disc_k_lam.pdf'])
            end
        end
        ...
        saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\goodness_metric_k_lam.png'])
        saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\goodness_metric_k_lam.pdf'])
    ...
    case 'criscione'
        ...
        for i = 1:length(output.k3)
            ...
            c.Label.String = 'log(h/sum(h(:)))';
            if i == 1
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_full_k2_k3.png'])
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_full_k2_k3.pdf'])
            elseif i ==2
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_disc_k2_k3.png'])
                saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\2Dhist_disc_k2_k3.pdf'])
            end
        end
        ...
        ylabel('$S_{metric}=\sum_{i,j}\hat{hist}\left(k_i,\lambda_j,test\right)\;.*\;\hat{S}_\alpha\left(k_i,\lambda_j,\alpha\right)$',...
            'interpreter','latex')
        legend(output.lbl{1},output.lbl{2},'interpreter','latex')
        saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\goodness_metric_k2_k3.png'])
        saveas(gcf,['Data\23-0714-sim-' fileName{ii} '\goodness_metric_k2_k3.pdf'])
end
end