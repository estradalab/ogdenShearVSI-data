function DESTE=ogden_deste_3d(varargin)
...
for nf=1:3
    ...
    % Half resolution = good; small deviation = bad

    % prelim(nf)=varianms(fid_directory, 'pixvec',[128 32 32],'apod', [58 14],'gradlag',48);
    % prelim(nf)=varianms(fid_directory, 'pixvec',[128 32 32],'apod', [64 16],'gradlag',48); %straight up reconstruction from fid, 
    % prelim(nf)=varianms(fid_directory, 'pixvec',[128 32 32],'apod', [128 32],'gradlag',48); %straight up reconstruction from fid, 
    % prelim(nf)=varianms(fid_directory, 'pixvec',[128 32 32],'apod', [8 2],'gradlag',48); %straight up reconstruction from fid, 
    prelim(nf)=varianms(fid_directory, 'pixvec',[128 32 32],'apod', [128 32]/0.9,'gradlag',48);
