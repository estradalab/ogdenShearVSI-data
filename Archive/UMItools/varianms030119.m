...
%'apod'     : apodization (smooth roll off) with numeric input options:
...
%'cut'      : 1, 2 or 3 entry input giving reduced pixel counts
...
%'cut'      : [ pix1 pix2 pix3] to define image size smaller than the acquired data 
...
if ~isempty(strfind(out.pars.seqfil,'ms'));    %case multislice acquisition
    ...
    %flip the axes to descending order
...
switch lower(switchstring)
    case {'sems'; 'semsdw'; 'stems';'stemsdw';'gems';'gemsvc'};%%%%%%%%%%%%%%%%%%%%%% single echo/FID multislice
        ...
        elseif ((numel(pc.ti) == rem)  || (numel(pc.te) == rem)); %straight inversion recovery or multi-te;
...
if any(strcmp(varargin,'apod')) && strcmp(acquisitiontype,'2D');
    ...
    if nargin==apodindex || ~isnumeric(varargin{apodindex+1}); %'apod' is last option or next option not numeric
        ...
        W(ktophat(:))=1;
        W(kring(:))=0;
    elseif nargin>apodindex && isnumeric(varargin{apodindex+1})&& numel(varargin{apodindex+1})==1; %'apod' is followed by a single number, apodize time domain only
        si=size(acquireddata);
        %kc=0.9;             %cutoff k
...
if strcmp(acquisitiontype,'2D');
    ...
    if exist('pixvec','var');
        if numel(pixvec) == 3;
            ...
            for jj=1:higherDimNumber;
                dummy=fftshift(fft(pic(:,:,:,jj),[],3),3);
