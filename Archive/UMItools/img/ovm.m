function ovm(threshold,threshold2, spm_file, spm_file2, anat_file )
...
global SPM_scale_factor 
    ...
    voxList = find( (spm_data2 >= threshold2) & (spm_data >=threshold));
    [vlx vly vlz] = ind2sub(size(spm_data), voxList);
    voxList = [vlx vly vlz];
    display('saving overlaps.dat')
    ...
    spm_dataB(find(spm_dataB <= threshold2 )) = 0;
