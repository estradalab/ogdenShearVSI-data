...
orientation='axial';
 ...
 elseif  vol.dim(2)==vol.dim(3) 
% probably sagittal            inpl_crd=[2 3];

 thrpl_crd=1;

            orientation='sagittal';

 elseif  vol.dim(1)==vol.dim(3) 
% probably coronal            inpl_crd=[1 3];
...
%thres = input('gimme threshold 
% [5] =');
%if(isempty(thres))  thres = 5;
...
% initialize the corrected output volume YcYc=zeros(vol.dim(1:3));
...
% ----------------------------for slice = 1:vol.dim(thrpl_crd);
 ...
 fprintf('\b
%d',slice);
      elseif slice<100,  fprintf('\b\b
%d',slice);
      elseif slice<1000, fprintf('\b\b\b
%d',slice);
    end    
...
% though no need here    Imax = max(max(img));
    Ithr = Imax*thres;
    
%  fill in holes    mask = (img>Ithr);
    Imask = img.*mask;
    Iave = sum(sum(Imask))/sum(sum(mask));
    Ifill = Imask + (1-mask).*Iave;
    
%  make low freq image    z = fftshift(fft2(Ifill));
...
% use 15 as default win for guassian    a2 = 1/n;
    for x=1:np2      r2 = y2 +x*x;
      win1(x,:) = exp(-a2*r2);
    end    win(np2+1:npix,np2+1:npix) = win1;
    win(np2+1:npix,np2:-1:1) = win1;
    win(1:np2,:) = win(npix:-1:np2+1,:);
    zl = win.*z;
    Ilow = abs(ifft2(fftshift(zl)));
    
%corrected image    Ilave = sum(sum(Ilow.*mask))/sum(sum(mask));
    Icorf = (Ilave./Ilow).*mask;
  
% correction factor    Iout = img.*Icorf;
