function nifti2analyze3D(name, doFlipX)
% function nifti2analyze3D(name [, doFlipX])
%
% converts NIFTI images to AVW (Analyze) format images disregarding
% the affine transformation parameters
%
% if desired, it Flips the data along the X axis WITHOUT CHANGING
% THE HEADER.  (It's an easy edit if you want to change it yourself - 
% just look for the comment that says - "change the header here" )
%
% this version writes a series of 3D files.
%
...
if doFlipX
    [T P]= size(dnii);
    tmp = dnii';
    tmp = reshape(tmp,  avwh.xdim,  avwh.ydim * avwh.zdim * avwh.tdim);
    tmp = tmp(end:-1:1,:);
    tmp = reshape(tmp, P , T);
    dnii = tmp';
end
...
for f=1:avwh.tdim
    str = sprintf('vol_%04d.img', f);
    write_img( str, dnii(f,:), outh);
end
...
if strcmp(computer,'GLNX86') | strcmp(computer , 'PCWIN') | strcmp(computer,'MACI') | strcmp(computer,'GLNXA64')
