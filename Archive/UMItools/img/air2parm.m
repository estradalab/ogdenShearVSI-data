function realign = air2parm()
% air2parm  script.
% it reads a time series of .air files and writes out the realign.dat file
% as usual, after computing the rotations and translations from the 
% affine transformation matrices
	...
	for count = 1:numFiles	
		tmp = readair(files(count).name);
		realign = [realign ; tmp];
	end

	save realign.dat realign -ASCII
