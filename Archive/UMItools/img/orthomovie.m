function result = orthomovie( roi,root, tsroot, x, y, z)
% function result = orthomovie( roi_size, root,   x, y, z)
% ...or    result=orthomovie()
%
...
% at x,y,z.
% 
% it does it over the whole time series showing you a movie!!
%
...
while k >= -10
    ...
    for count=1:size(wholedata,1)
        
        d = reshape(wholedata(count,:), h.xdim, h.ydim, h.zdim);
        dd = (d-min(min(min(d))))*256/range;
        if ~isempty(wscale)
            dd = (d-wscale(1))*256 / wscale(2);
        end
