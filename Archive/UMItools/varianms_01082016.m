...
%'iso'      : zerofill (if time domain) or interpolate (if images) to make isotropic images and pixels
...
if strcmp(inputtype{1}, 'timedomain');          %a filename or directory for the fid was handed down
    ...
    if any(strcmp('iso',varargin));
        ...
        out.pars.lpe=largest_FOV;
        out.pars.lro=largest_FOV;
        for ns=1:nsl;
            src=zeros(size(Xtmp));
...
if any(strcmp(varargin,'upsample')) || any(strcmp(varargin,'1k')) || any(strcmp(varargin,'1K'));
    ...
    for ns=1:numsl;
        ...
        if any(strcmp(varargin,'1k')) || any(strcmp(varargin,'1K'));
            display(['Setting image ' num2str(ns) 'to 1k resolution in varianms....']);
        else
...
switch lower(pc.seqfil)
    ...
    case {'mems'; 'mgems';'mems_us';'mgems_us';'dwsemgems_us';'dwgesege_us'; ...
			'dwgesege_us_eq';'dwgesege_us_drodpedsl';'gesege_spoil';'dwgs1';'dwssgg1'};%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% multiecho multislice (T2-weighting)
        ...
        if rem==numel( eval(['pc.' pc.array])); %the FID is blocked, and varian writes blocks into the second dimension
